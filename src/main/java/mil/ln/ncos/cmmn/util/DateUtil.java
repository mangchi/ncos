...
public class DateUtil {
	...
	public static String convertDate(String strSource, String fromDateFormat, String toDateFormat, String strTimeZone) {
		...
		if (StringUtil.isNullToString(fromDateFormat).trim().equals(""))
			sFromDateFormat = "yyyyMMddHHmmss"; // default값
		if (StringUtil.isNullToString(toDateFormat).trim().equals(""))
			sToDateFormat = "yyyy-MM-dd HH:mm:ss"; // default값

		try {
			simpledateformat = new SimpleDateFormat(sFromDateFormat, Locale.getDefault());
			...
			simpledateformat = new SimpleDateFormat(sToDateFormat, Locale.getDefault());
	...
	public static String formatDate(Date date, String fmt) {
		...
		SimpleDateFormat formatter = new SimpleDateFormat(fmt, Locale.KOREA);  
		return formatter.format(date);  
	}

    
	public static String formatDate(String sDate, String ch, boolean isNull) {
		if (isNull && String.valueOf(sDate).trim().equals("")) {
			return "";
		} else {
			return formatDate(sDate, ch);
		}
	...
	* ex) 20040101,ch(/) --> 2004/01/01 로 리턴
	...
	public static String getCurrentDate(String... str) {
		...
		String dateType = str[0];
		String format = "yyyyMMdd";
		if (str.length > 1) {
			format = str[1];
		}
		if (!"".equals(dateType))
			strDate = convertDate(strDate, format, dateType);
	...
	public static boolean validDt(String sDate) {
	...
	public static String validChkDate(String dateStr) {
		...
		if (dateStr.length() == 10) {
			sDateStr = StringUtil.removeMinusChar(dateStr);
		}
		return sDateStr;
	}
	...
	public static String validChkTime(String timeStr) {
		String sTimeStr = timeStr;

		if (sTimeStr.length() == 5) {
			sTimeStr = StringUtil.remove(sTimeStr, ':');
		}
		if (sTimeStr == null || !(sTimeStr.trim().length() == 4)) {
			throw new IllegalArgumentException("Invalid time format: " + sTimeStr);
		}
	...
	public static int getMonth(Timestamp ts) {
		SimpleDateFormat sdf = new SimpleDateFormat("yyyMMdd", Locale.KOREA);
		return Integer.parseInt(sdf.format(ts).substring(4, 6));
	}
	
	public static int getYear(Timestamp ts) {
		SimpleDateFormat sdf = new SimpleDateFormat("yyyMMdd", Locale.KOREA);
		return Integer.parseInt(sdf.format(ts).substring(0, 4));
	}
	
	public static int getDay(Timestamp ts) {
		SimpleDateFormat sdf = new SimpleDateFormat("yyyMMdd", Locale.KOREA);
		return Integer.parseInt(sdf.format(ts).substring(6, 8));
	}
	public static String getFrmtDate(Timestamp ts,String frmt) {
		if(ts == null) {
			ts=new Timestamp(System.currentTimeMillis());  
		}
		SimpleDateFormat sdf = new SimpleDateFormat(frmt, Locale.KOREA);
		return sdf.format(ts);
	}
	
	public static long getUnxiDate(String dateStr,String frmt) throws Exception{
		if(frmt == null) {
			frmt = "yyyyy-MM-dd HH:mm:ss";
		}
		SimpleDateFormat dt = new SimpleDateFormat(frmt,Locale.getDefault()); 
		//dt.setTimeZone(java.util.TimeZone.getTimeZone("GMT+9")); 
